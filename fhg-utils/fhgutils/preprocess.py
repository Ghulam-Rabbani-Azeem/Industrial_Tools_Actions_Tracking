...
def filter_labels(labels, Xt, Xc, y):
    ...
    mask = np.isin(y_labels, labels, invert=True)  # mask are only the values that we do consider, maasks true if not -1
    ...
    print("[INFO] filtered yt:", y_filtered.shape)
    return Xt_filtered, Xc_filtered, y_filtered # returns only the clean windows


def one_label_per_window(y): # len(y) - length of the window
    # get labels per window
    y_labels = []
    for i in range(len(y)):
        values, counts = np.unique(y[i], return_counts=True) # takes the first window and returns the unique number of labels and their counts
        idx = np.argmax(counts) # returns the index of the label that occurs maximum
        # take label only if it occurs more than 50% of the window
        if counts[idx] > 0.5 * np.sum(counts):
            y_labels.append(int(values[idx])) # appending the majority values as the label and append to y labels list
    print("Below find the number of unique labels in the data")
